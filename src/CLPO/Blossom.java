...
/* Implementation of the Edmond's blossom matching algorithm,
   adapted by the JANPA team from
   https://github.com/simlu/EdmondsBlossom (according to its MIT license),
   which, in turn, is based on a Python (and also Perl) implementation 
   from http://jorisvr.nl/maximummatching.html 
   Below is also a comment from original Java version:
...
public class Blossom {
    ...
    public Blossom(PairEdge[] edges){//, int nvertex) {
        ...
        for (int i = 0; i < nedge; i++) {
            endpoint[p++] = edges[i].a;
            endpoint[p++] = edges[i].b;
        }
/*        
        ...
        for (int k = 0; k < nedge; k++) {
            neighbend[edges[k].a].add(2 * k + 1);
            neighbend[edges[k].b].add(2 * k);
        }
        ...
        // problem.
        double[] dualvar = new double[nvertex * 2];
        for (int i = 0; i < nvertex; i++) {
            dualvar[i] = maxweight;
        }
    ...
    private double slack(int k) {
        return dualvar[edges[k].a] + dualvar[edges[k].b] - 2L * edges[k].wt;
    }
            ...
            while (true) {
                ...
                while (!queue.isEmpty() && augmented == 0) {
                    ...
                    for (int p : neighbend[v]) {
                        ...
                        double kslack = Double.MAX_VALUE; // ???? // = Long.MAX_VALUE;
                        if (!allowedge[k]) {
                            kslack = slack(k);
                            if (kslack <= 0) {
                ...
                for (int b = 0; b < 2 * nvertex; b++) {
                    if (blossomparent[b] == -1 && label[b] == 1 && bestedge[b] != -1) {
                        double kslack = slack(bestedge[b]);
                        double d = kslack / 2;
                        if (deltatype == -1 || d < delta) {
                            delta = d;
                            deltatype = 3;
